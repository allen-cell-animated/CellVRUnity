...
Shader "Custom/Volume1" 
{
	...
	SubShader 
	{
		...
		// constant values
		float3 AABB_CLIP_MIN = float3(-0.5,-0.5,-0.5);
		float3 AABB_CLIP_MAX = float3(0.5,0.5,0.5);
		int BREAK_STEPS = 72;
		float BRIGHTNESS = 1;
		float DENS = 0.0820849986238988;
		int ATLAS_X = 8;
		int ATLAS_Y = 6;
		int SLICES = 46;
		float maskAlpha = 0;
		...
		{
			uv = In.pObj.xy;
			float3 eyeRay_o = mul(unity_WorldToObject, float4(_WorldSpaceCameraPos, 1.0)).xyz; // cameraPosition = _WorldSpaceCameraPos
			...
			if (tnear > tfar) //(tfar > -0.5 && tfar < 0.1) (tnear > 1.1 && tnear < 1.2) 
			{
